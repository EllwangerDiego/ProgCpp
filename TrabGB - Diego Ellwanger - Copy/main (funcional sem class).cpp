...
public:
    Registro(string linha)
    {
        ...
        {
            if (ehNumero(restoTokens[i]))
            {
                prioridade = stringParaInt(restoTokens[i]);
                prioridadeIndex = i;
                break;
            }
        }
        ...
        protocolo = (prioridadeIndex + 1 < (int)restoTokens.size()) ? restoTokens[prioridadeIndex + 1] : "";
        origemIP = (prioridadeIndex + 2 < (int)restoTokens.size()) ? restoTokens[prioridadeIndex + 2] : "";
    ...
    // Método para exportar em formato texto linha original (ou formatada)
